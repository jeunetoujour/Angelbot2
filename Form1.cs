...
namespace AngelBot
{
    ...
    public partial class Form1 : Form
    {
        ...
        public int reactionInfoAddress = 0xB59EF0;//0xB511D0;//0xB3C0C4 + 0x184;  B58E6C
        public int resInfoAddress = 0xB59EA8;
        public int sellInfoAddress = 0xB510C0;
        public int vendorInfoAddress = 0xB510A4;
        public int gatheringInfoAddress = 0xB59DCC;//0xB3C0C4 + 0x60;
        public int chatInfoAddress = 0xB59D3C;//0xB3C0C4 + 0x60 + 0x38;
        public uint gatheroffset = 0xB59DCC;
        ...
        public void getattacklist()//Adds attacks to list
        {
            if (File.Exists(pc.Name + ".skills"))
            {
                ...
                if (Attacklist.GetNodeCount(true) < 6) MessageBox.Show("Please add skills in skill editor");
                sr.Close();
                file.Close();
                loadnewheals();
            }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            Keys w = Keys.W | Keys.Control;
            WindowsShell.RegisterHotKey(this, k);
            WindowsShell.RegisterHotKey(this, w);
            ...
            if (pc.Level > 30 && (f11.test == "pmet" || f11.temp == false))
            {
                MessageBox.Show("This is a Demo for levels 30 and under. Please purchase at http://www.angelsuite.net/buy");
                Process.Close();
                Environment.Exit(0);
            }
#endif
        ...
        */
        ...
        private void mainattackloop()//NEW ATTACKLOOP
        {
            ...
            if (pc.TargetID != pc.ID || killsteal == true) //&& (targetstar1.ID == pc.ID || targetstar1.Type == eType.FriendlyNPC || tar.HasTarget != true)) //selftarget
            {
                ...
                while (tar.Stance != eStance.Dead && pc.Health != 0 && btnstop.Visible == true) //Top level 
                {
                    ...
                    if (prevskill != "") prevskill = skillname;

                    skillname = Attacklist.SelectedNode.Text;
                    ...
                    if (delay < 200)
                    {
                        delay = 400; //MessageBox.Show("Skill detected with < 200ms! Fix skills!");
                    }
                    skillready(skillname);
                    if (abilities[skillname].AbilityID == 50006) delay = 150; //attack/chat
                    ...
                    if (tar.TargetID == pc.ID || tar.TargetID == petid) tmrstuck.Stop(); //if mob is on me, no antistuck
                    ...
                    if (abilities[skillname].AbilityID == 50006 && maxtries < 2) maxtries = 2; //Attack/chat
                        ...
                        else
                        {
                            ...
                            if (skillready(skillname) == false) //skill was fired and is on cooldown
                            {
                               ...
                               // listkeypress.Items.Add(skillname + " Fired");
                                ...
                                if (optimizer == true && prevskill != "" && (abilities[skillname].AbilityID != 50006))
                                {
                                    abilities[prevskill].CastTime -= 10;  //Attacklist.SelectedNode.Text
                                    abilities[prevskill].CastTimeMod -= 10;
                                    listkeypress.Items.Add(skillname + " Fired: -10ms to " + prevskill);
                            ...
                            else
                            {
                                ...
                                if (optimizer == true && prevskill != "" && (abilities[skillname].AbilityID != 50006))
                                {
                                    ...
                                    listkeypress.Items.Add(skillname + " Didn't Fire: +10ms to " + prevskill); 
                                    listkeypress.Items.Add(skillname + " Casttime:" + abilities[Attacklist.SelectedNode.Text].CastTime);
                                    if (listkeypress.Items.Count > 0) listkeypress.SetSelected(listkeypress.Items.Count - 1, true);
                                }
                                else { 
        ...
        private void UpdateReactions()
        {
            ...
            RControlList = AddControl(Memory.ReadInt(Process.handle, (uint)((Process.Modules.Game + reactionInfoAddress))), -1, RControlList); //A657D0  
        ...
        private object[,] AddControl2(long address, int parent, object[,] ControlList)
        {
            long newcontrolname = address;
            //int newcontrolname = (int)Memory.ReadUInt(Process.handle, (uint)(address));
            long controlname = (long)Memory.ReadSByte(Process.handle, (uint)(newcontrolname + 0x1C));
            string scontrolname = "";
            byte controlname2 = 0;
            int scontrolname1 = 0;
            double posx = 0, posy = 0;
            ...
            for (int i = 0; i < ControlList.GetLength(0); i++)
            {
                if (Convert.ToInt64(ControlList[i, 1]) == 0)
                {
                    ControlList[i, 0] = parent;
                    ControlList[i, 1] = address; //address
                    ControlList[i, 2] = scontrolname; //chainname
        ...
        */
        ...
        private void attackmob()
        {
            ...
            if (isranged == true)
            {
                if (tar.Distance3D(pc) >= rangedist)
                {
                    return;
                }
            }
        ...
        */
        ...
        public bool adddetect()
        {
            ...
            foreach (Entity ents in elist)
            {
                ...
                {
                    if ((ents.TargetID == pc.ID || ents.TargetID == petid)&& ents.Health > 0 && ents.Distance3D(pc.X, pc.Y, pc.Z) < 40)
                    {
                        ...
                        if ((tar.TargetID != pc.ID  || ents.TargetID != petid) && tar.ID != 0 && keypressing == false) keyenumerator("ESC");
                        if (savelog == true) tw.WriteLine(DateTime.Now + " ADD found in ADD detection: " + ents.Name + " Distance: " + pc.Distance3D(ents));
                        tar.UpdateID();

                        while (tar.TargetID != pc.ID || tar.TargetID != petid)
                        {
                            if(pc.Class == eClass.Spiritmaster)
                                if(petid > 0) keyenumerator(keytarget);
        ...
        public void sellmisc()
        {
            ...
            nControlList = AddControl(Memory.ReadInt(Process.handle, (Process.Modules.Game + vendorInfoAddress)), -1, nControlList); //set control list initial npc dialog
            int i = 0;
            for (i = 0; i <= 30 - 1; i++)
            {
            ...
            nControlList = AddControl(Memory.ReadInt(Process.handle, (Process.Modules.Game + sellInfoAddress)), -1, nControlList); //set control list for vender.
            //int i = 0;
            for (i = 0; i <= 30 - 1; i++)
            {
                if (Convert.ToInt32(nControlList[i, 1]) != 0)
        ...
        public void resurrect()
        {
            ...
            nControlList = AddControl(Memory.ReadInt(Process.handle, (Process.Modules.Game + resInfoAddress)), -1, nControlList); //Resurrection
            ...
            while (pc.Health == 0)
            {
                ...
                if (offsety > 30)
                {
                    offsety = -5;
                    offsetx += 5;
                    if (offsetx > 182) offsetx = -10;
                    //keyenumerator("ENTER");
                    okbuttonx = Convert.ToInt32(aionscreen.Left + Convert.ToDouble(nControlList[0, 5])) + 32 + offsetx + Convert.ToInt32(nControlList[2, 5]) + 32; //+ Convert.ToInt32(nControlList[2, 5]));
                }
            }
        ...
        private bool mobinrange()
        {
            if (isranged == true)
            {
                double dis = tar.Distance3D(pc);
                if (dis <= rangedist)
                {
                    return true;
                }
        ...
        private void findmob()
        {
            ...
            if (isresting == false)
            {
                ...
                while (mobinrange() == false || (deathrun == true && defenddeath == false) || tar.Type != eType.AttackableNPC || (((int)tar.Level <= ignorelevel) || hasplayer == true || ignorelist.Contains(tar.Name) || ignorelist.Contains(tar.ID.ToString())))
                {
        ...
        public void changewaypoint()
        {
            ...
            try
            {
                ...
                else if (venderrun != true)
                {
                    ...
                    if (pc.Distance3D(awaypoint.X, awaypoint.Y, awaypoint.Z) < 2.7) //hit waypoint CHANGE THIS
                    {
                        if (movecounter <= 0)
                        {
                            forward = true;
            ...
            {
                System.Diagnostics.StackTrace trace = new System.Diagnostics.StackTrace(emp, true);
               
                MessageBox.Show("Error in changewaypoint line: " + trace.GetFrame(0).GetFileLineNumber() + " rest:" + emp);
            }
        ...
        public void waymovement(bool ontrack)
        {
            ...
            //if(Math.Abs((x) - (temp)) <= 3 && Math.Abs((x) - (temp)) > 0.2)

            if (Math.Abs((x) - (temp)) > 0.2)
               pc.WriteRot((float)awaypoint.Face(pc.Y, pc.X));
            //label11.Text = x.ToString() + " : " + temp.ToString();
...
*/
        ...
        /* do
      {
        ...
        {
            long dialogptr = Memory.ReadUInt(Process.handle, (uint)(Process.Modules.Game + 0xB58F60)); //after target_delay_dialog (8) to (C)
            byte status = Memory.ReadByte(Process.handle, (uint)(dialogptr + 0x24));
            long dialogptr2 = Memory.ReadUInt(Process.handle,(Process.Modules.Game + 0xB58F60 +0x8)); //after target_delay_dialog (8) to (C)
            byte status2=0;
            
            if (dialogptr2 != 0)
                status2 = Memory.ReadByte(Process.handle, (uint)(dialogptr2 + 0x24));


            if (status == 175 || status2 == 175)
            {
                TakeOver aion = new TakeOver((int)hwndAion);
                RECT aionscreen = new RECT();

                //PauseForMilliSeconds(600); //wait for the death
                
                if (savelog == true) tw.WriteLine(DateTime.Now + " QItem Dialog clicking started");
                ...
                object[,] nControlList = new object[8, 7];
                for (int b = 0; b < 8; b++)
                {
                    nControlList[b, 0] = 0; //parent
                    nControlList[b, 1] = 0; //address
                ...
                int off = 0x0;
                if(status == 175)
                    nControlList = AddControl2((long)Memory.ReadUInt(Process.handle, (uint)(Process.Modules.Game + 0xB3C32C)), -1, nControlList);
                if(status2 == 175)
                    nControlList = AddControl2((long)Memory.ReadUInt(Process.handle, (Process.Modules.Game + off + 4)), -1, nControlList); 

                int i = 0;

                if (Convert.ToInt64(nControlList[i, 1]) != 0)
                {
                    //int baseoffset = Memory.ReadInt(Process.handle, (Convert.ToInt32(nControlList[i, 1])));
                    long baseoffset = Convert.ToInt64(nControlList[i, 1]);
                    int childlist = (int)Memory.ReadUInt(Process.handle, (uint)(baseoffset + 0x1C0));
                    int childsize = (int)Memory.ReadUInt(Process.handle, (uint)(baseoffset + 0x1C4));

                    long thiscontrol = (long)Memory.ReadUInt(Process.handle, (uint)(childlist + 0x4));
                    for (int j = 1; j <= childsize - 1; j++)
                    {
                        thiscontrol = (long)Memory.ReadUInt(Process.handle, (uint)(thiscontrol + 0x4));
                        long final = (long)Memory.ReadUInt(Process.handle, (uint)(thiscontrol + 0x8));
                        nControlList = AddControl2(final, i, nControlList);
                    }
                }
                if (nControlList[2, 2].ToString() != "ok") return; //not right window!
                ...
                if (status == 175)
                 while (Memory.ReadByte(Process.handle, (uint)(dialogptr + 0x24)) == 175)
                 {
                ...
                if(status2 == 175)
                    while (Memory.ReadByte(Process.handle, (uint)(dialogptr2 + 0x24)) == 175)
                    {
