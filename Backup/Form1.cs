...
namespace AngelBot
{
    ...
    public partial class Form1 : Form
    {
        ...
        object[,] RControlList = new object[12, 7];
        ...
        public Form1()
        {
            try
            {
                ...
                abilities.HOTBAR_OFFSET = aionOffsets.hotkeyInfoAddress;
        ...
        */
            ...
            foreach (string healname in heals) //get a heal that is ready and meets criteria
            {
                ...
                if (skillready(healname) == true && abilities[healname].CastPercent >= healthpercentage)
                {
                    while (numtries < 3 && skillready(healname) == true)
                    {
                        ...
                        if (savelog == true) tw.WriteLine(DateTime.Now + " Casting Heal #" + healcounter + ">> " + "Key: " + key + " HP: " + pc.Health + "/" + pc.MaxHealth);
                        keyenumerator(key);
                        ...
                        if (skillready(healname) == true) listkeypress.Items.Add(healname + " Not Fired");
                    }
        ...
        private void mainattackloop()//NEW ATTACKLOOP
        {
            ...
            if (pc.TargetID != pc.ID || killsteal == true) //&& (targetstar1.ID == pc.ID || targetstar1.Type == eType.FriendlyNPC || tar.HasTarget != true)) //selftarget
            {
                ...
                while (tar.Stance != eStance.Dead && pc.Health != 0 && btnstop.Visible == true) //Top level 
                {
                    ...
                    if (tar.TargetID == pc.ID) //doesnt have me targetted
                    {
                        tmrstuck.Stop();
                        stuckcounter = 0;
                        returncounter = 0;
        ...
        private bool chainskillready(string skillname) //this function checks if a % chance chain is up
        {
            ...
            if (abilities[skillname].PerChance == false) return true;
            return false;
        }
        ...
        {
            UpdateReactions();
            for (int r = 11; r >= 6; r--) //reaction range
            {
        ...
        private object[,] AddControl(int address, int parent, object[,] ControlList)
        {
            ...
            else if (controlname > 0)
            {
                ...
                scontrolname1 = (int)Memory.ReadUInt(Process.handle, ((uint)(newcontrolname + 0x2D8)));
            }
            else
            {
        ...
        */
        ...
        public string findgatherable()
        {
            ...
            {
                if (ents.Type == eType.Player && ents.Distance2D(pc.X, pc.Y) < gatherdistance && ents.Name != pc.Name) return "";
                if ((gatherlist.Count > 0 && gatherlist.Contains(ents.Name)) || gatherlist.Count == 0)
                {
                    if (ents.Type == eType.Gatherable && ents.Distance2D(pc.X, pc.Y) < gatherdistance)
                    {
        ...
        public void startbtn()
        {
            ...
            if (tar.Type == eType.AttackableNPC || tar.Type == eType.Gatherable)
            {
                SetForegroundWindow(hwndAion);
                getpcptr();
                PauseForMilliSeconds(50);
        ...
        private void looting()
        {
            ...
            while (tar.Type == eType.DeadwLoot && ignorelist.Contains(tar.ID.ToString()) == false) //ADD INVENTORY FULL CHECK
            {
                ...
                if (savelog == true) tw.WriteLine(DateTime.Now + " Total Exp gained: " + lblxpgain.Text);
                killLabel.Text = "Kills: " + kills;
        ...
        private void findmob()
        {
            ...
            if (isresting == false)
            {
                ...
                while ((deathrun == true && defenddeath == false) || tar.Type != eType.AttackableNPC || (((int)tar.Level <= ignorelevel) || hasplayer == true || ignorelist.Contains(tar.Name) || ignorelist.Contains(tar.ID.ToString())))
                {
                    ...
                    if ((istabbing == false) && (deathrun == false) && (venderrun == false))
                    {
                        PauseForMilliSeconds(100);
                        istabbing = true;
        ...
        public void changewaypoint()
        {
            if (deathrun == true)
            {
                if (pc.Distance3D(awaypoint.X, awaypoint.Y, awaypoint.Z) < 3) //hit waypoint CHANGE THIS
                {
                    deathcounter++;
                }
            ...
            {
                if (pc.Distance3D(awaypoint.X, awaypoint.Y, awaypoint.Z) < 2.8)
                {
        ...
        /* do
      {
        ...
        public void gather()
        {
            ...
            if (gathername != "" && adddetect() == false)
            {
                ...
                //antistuck = true;
                if (antistuck == true) tmrstuck.Enabled = true;
                while (tar.Distance2D(pc.X, pc.Y) > 5.5) //wait loop while getting in range
                {
                    PauseForMilliSeconds(1000);
                    if (adddetect() == true)
                        return;
                ...
                if (antistuck == true) tmrstuck.Enabled = false;
                PauseForMilliSeconds(600);
                while (gathername != "")
                {
                    while (Memory.ReadByte(Process.handle, (uint)(chatinput + 0x24)) == 7)
