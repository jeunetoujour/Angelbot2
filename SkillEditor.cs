...
namespace AngelBot
{
    public partial class SkillEditor : Form
    {
        ...
        /*private void button27_Click(object sender, EventArgs e)
        {
            ...
            abilities.ABILITY_OFFSET = f1.aionOffsets.abilityInfoAddress;
            abilities.Update();
        ...
        */
        ...
        private void listBox4_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            textBox1.Text = (abilities[textBox20.Text].CastTime + abilities[textBox20.Text].CastTimeMod).ToString();
            label6.Text = abilities[textBox20.Text].AbilityID.ToString();
            best = false;
        }
        ...
        private void SkillEditor_Load(object sender, EventArgs e)
        {
            ...
            //abilities.ABILITY_OFFSET = f1.abilityInfoAddress;
            //abilities.HOTBAR_OFFSET = f1.hotkeyInfoAddress;
            abilities.Update();

            foreach (Ability item in abilities)
            { //IsAlphaNumeric(item.AbilityName) == false &&
                if ((item.AbilityName.Contains("Toggle") == false) && (item.AbilityName.Contains("Basic") == false) && (item.AbilityName.Contains("Advanced") == false))
                {
                    if(item.AbilityName != "Nothing")
                    Abilitylist.Add(item.AbilityName);
                }
                else
                {
        ...
        public void FillTree()
        {
            ...
            TreeNode mainBNode = new TreeNode();
            mainBNode.Name = "mainNode" + skilltree.Nodes.Count;
            mainBNode.Text = "SM Pet";
            this.skilltree.Nodes.Add(mainBNode);
        }
        ...
        {
            if (textBox1.Text == "0") MessageBox.Show("Total casttime for skill is 0!!! Fix it");
            if (skilltree.SelectedNode.Level != 0)
            {
                best = true; //flag for not updating txts
            ...
            else { checkBox1.Visible = false; checkBox1.Checked = false; }
            best = false;
        }
        ...
        private void updateskill()
        {
            if (skilltree.SelectedNode.Level > 0)
            {
                ...
                abilities[textBox20.Text].PerChance = checkBox1.Checked;
                textBox1.Text = (abilities[textBox20.Text].CastTime + abilities[textBox20.Text].CastTimeMod).ToString();
                if (abilities[textBox20.Text].CastTime + abilities[textBox20.Text].CastTimeMod < 200) MessageBox.Show("You need a cast time delay!");
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            if (best == false)
            {
                if (hotkey.Text == "" && checkBox1.Checked == true) hotkey.Text = abilities[skilltree.SelectedNode.Parent.Text].Keypress;
                updateskill();
            }
        }

        private void txtcasttimemod_TextChanged(object sender, EventArgs e)
        {

        }
        private void txtcasttimemod_Leave(object sender, EventArgs e)
        {
            if (best == false)
            { updateskill(); }
        }
        ...
        private void txtcastpercent_TextChanged(object sender, EventArgs e)
        {

        }
        private void txtcastpercent_Leave(object sender, EventArgs e)
        {
            if (best == false)
            { updateskill(); }
        }

        private void optimizer_CheckedChanged(object sender, EventArgs e)
        {
            if (optimizer.Checked == true) f1.optimizer = true; else f1.optimizer = false;
        }
    }
}
